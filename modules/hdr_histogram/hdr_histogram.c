    ...
    struct hdr_histogram* h, int32_t index, int64_t value)
{
	...
	as_add_int64(&h->counts[normalised_index], value);
    //hdr_atomic_add_fetch_64(&h->total_count, value);
	as_add_int64(&h->total_count, value);
}
...
static void update_min_max_atomic(struct hdr_histogram* h, int64_t value)
{
    ...
    do
    {
		...
		current_min_value = as_load_int64(&h->min_value);
    ...
    do
    {
		...
		current_max_value = as_load_int64(&h->max_value);
