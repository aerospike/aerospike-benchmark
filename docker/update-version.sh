...
# update-version.sh
#
# Updates the version, download URLs, and SHA256 checksums in the asbench Dockerfiles.
#
# JFrog URL structure:
#   DEB: {deb-base}/pool/{ubuntu-codename}/aerospike-asbench/{filename}
#        e.g. .../pool/noble/aerospike-asbench/aerospike-asbench_2.2.5-rc1_ubuntu24.04_x86_64.deb
#
# Usage:
#   ./docker/update-version.sh --version VERSION [OPTIONS]
#
# Required:
#   --version VERSION       asbench version, e.g. 2.2.5 or 2.2.5-rc1
#
# Source options:
...
#                           Use --deb-base-url to set the URLs written into Dockerfiles
#                           (pointing to where packages will be hosted).
#
...
#                           Full URL: {deb-base}/pool/{codename}/aerospike-asbench/{filename}
#
# Other options:
...
#   ./docker/update-version.sh --version 2.2.5-rc1 --packages-dir /path/to/pkgs
#
#   # Remote URL, no SHA256 (fastest):
#   ./docker/update-version.sh --version 2.2.5-rc1
#
#   # Remote URL + download to compute SHA256:
#   ./docker/update-version.sh --version 2.2.5-rc1 --compute-sha
#
#   # Preview without modifying files:
#   ./docker/update-version.sh --version 2.2.5-rc1 --packages-dir /path/to/pkgs --dry-run
...
# Package filenames
# ---------------------------------------------------------------------------
PKG_UBUNTU2404_AMD64="aerospike-asbench_${VERSION}_ubuntu24.04_x86_64.deb"
PKG_UBUNTU2404_ARM64="aerospike-asbench_${VERSION}_ubuntu24.04_aarch64.deb"

# ---------------------------------------------------------------------------
# Construct download URLs
# ---------------------------------------------------------------------------
DEB_POOL="${DEB_BASE_URL}/pool/${UBUNTU2404_CODENAME}/aerospike-asbench"
...
update_dockerfile() {
    ...
    sed -i.bak \
        -e "s|^ARG ASBENCH_AMD64_URL=.*|ARG ASBENCH_AMD64_URL=\"${amd64_url}\"|" \
        -e "s|^ARG ASBENCH_AMD64_SHA256=.*|ARG ASBENCH_AMD64_SHA256=\"${amd64_sha}\"|" \
        -e "s|^ARG ASBENCH_ARM64_URL=.*|ARG ASBENCH_ARM64_URL=\"${arm64_url}\"|" \
        -e "s|^ARG ASBENCH_ARM64_SHA256=.*|ARG ASBENCH_ARM64_SHA256=\"${arm64_sha}\"|" \
        "${file}"
    rm -f "${file}.bak"
    echo "  Updated: ${file}"
}
...
if [[ "${DRY_RUN}" == true ]]; then
    ...
    if [[ "${SHA_UBUNTU2404_AMD64}" == "PLACEHOLDER" || \
          "${SHA_UBUNTU2404_ARM64}" == "PLACEHOLDER" ]]; then
